...
| `azure` | All chat models from OpenAI that are hosted on Azure are supported. Requires `AZURE_OPENAI_API_KEY` and `AZURE_OPENAI_RESOURCE_NAME` environment variables. |
...
- `AZURE_OPENAI_RESOURCE_NAME`: This the resource name which is used to create the base URL with the format `https://$AZURE_OPENAI_RESOURCE_NAME.openai.azure.com`
