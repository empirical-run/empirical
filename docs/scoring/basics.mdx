...
- `json-syntax`: Returns 1 if output is a valid JSON object, 0 otherwise
- `sql-syntax`: Returns 1 if output is a valid SQL query string, 0 otherwise

## Custom scorers

There are two ways to build a custom scorer.

- `llm-critic`: Let an LLM score your output, based on your criteria ([configure this](./llm))
