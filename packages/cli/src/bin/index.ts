...
program
  ...
  .description('CLI to compare and evaluate multiple AI model completions on different prompts and model')
  .version(packageJSON.version);

program.command('init')
  .description('initialise empirical')
  .action(() => {
        fs.writeFileSync(`${path}/empirical.run.json`, JSON.stringify(config, null, 2))
        console.log(`${green('[Success]')} - created ${bold('empirical.run.json')} in ${path}`)
    });
